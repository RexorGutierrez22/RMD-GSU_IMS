...
const AnalyticsDashboard = () => {
	...
	return (
		<div className="w-full h-full">
			{/* Top Metrics - 6 Cards */}
			<div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-0 mb-1">
				{[
					{ title: 'STUDENTS', subtitle: 'Registered users', value: animatedStats.students, icon: '👨‍🎓', bg: 'from-blue-500 to-blue-600', color: 'blue' },
					{ title: 'EMPLOYEES', subtitle: 'Active staff', value: animatedStats.employees, icon: '👥', bg: 'from-green-500 to-green-600', color: 'green' },
					{ title: 'BORROWED', subtitle: 'Currently out', value: animatedStats.borrowed, icon: '📦', bg: 'from-orange-500 to-orange-600', color: 'orange' },
					{ title: 'LOW STOCK', subtitle: 'Needs attention', value: animatedStats.lowStock, icon: '⚠️', bg: 'from-red-500 to-red-600', color: 'red' },
					{ title: 'AVAILABLE', subtitle: 'Ready to use', value: animatedStats.available, icon: '✅', bg: 'from-emerald-500 to-emerald-600', color: 'emerald' },
					{ title: 'TOTAL', subtitle: 'All items', value: animatedStats.total, icon: '📊', bg: 'from-purple-500 to-purple-600', color: 'purple' }
				].map((metric, index) => (
					<div key={index} className={`bg-gradient-to-br ${metric.bg} p-2 shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300 text-white border-r border-white/20 last:border-r-0`}>
			...
			{/* Main Content Grid */}
			<div className="grid grid-cols-1 lg:grid-cols-4 gap-0 mb-1 flex-1">
				{/* Borrowing Trends Chart - Takes 2 columns */}
				<div className="lg:col-span-2 bg-white shadow-lg p-3 border-r border-gray-200">
				...
				{/* Inventory Distribution - Takes 1 column */}
				<div className="bg-white shadow-lg p-3 border-r border-gray-200">
					<h3 className="text-lg font-bold text-gray-800 mb-1">Inventory</h3>
				...
				<div className="bg-white shadow-lg p-3">
					...
					<div className="space-y-1 max-h-32 overflow-y-auto">
						{[
							{ type: 'warning', message: 'Markers', detail: 'Out of stock', color: 'bg-red-50 border-red-200' },
							{ type: 'info', message: 'Paper', detail: 'Low stock', color: 'bg-yellow-50 border-yellow-200' },
							{ type: 'success', message: 'Shipment', detail: 'Delivered', color: 'bg-blue-50 border-blue-200' }
						].map((alert, index) => (
							<div key={index} className={`p-2 rounded border ${alert.color}`}>
								<div className="text-xs font-medium text-gray-800">{alert.message}</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-0">
				{[
					...
					<div key={index} className={`${item.bg} shadow-lg p-3 hover:shadow-xl transition-all duration-300 border-r border-gray-200 last:border-r-0`}>
						<div className="flex items-center justify-between">
							<div className="flex-1">
								...
								<div className="text-xs text-gray-500 mt-1">{item.percentage} of total inventory</div>
							</div>
